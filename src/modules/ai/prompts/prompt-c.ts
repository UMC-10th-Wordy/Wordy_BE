...
지시 3: keyAchievement에는 이번 주 업무 중 가장 중요하거나 의미 있는 주요 성과를 1~2개 작성하세요. 반드시 입력된 성과 데이터에 근거해야 합니다.
지시 4: focusedTagId에는 이번 주에 가장 많은 업무와 성과가 집중된 프로젝트 태그의 tagId를 반환하세요. 반드시 입력 데이터에 존재하는 tagId 중 하나만 선택하세요.
지시 5: focusedTagName에는 focusedTagId에 해당하는 태그의 tagName을 그대로 반환하세요. focusedTagId와 focusedTagName은 반드시 동일한 프로젝트 태그를 가리켜야 합니다.
지시 6: 입력 데이터에 프로젝트 태그가 존재하지 않는 경우 focusedTagId와 focusedTagName은 null로 반환하세요.
지시 7: KPI 이름과 목표는 입력된 값을 그대로 사용하세요. 새로운 KPI를 만들지 말고, 진행 상황 설명만 생성하세요.
지시 8: 프로젝트(태그)별 목표 대비 진행 상황과 예상 결과를 분석하세요.
